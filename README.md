...
For wildcard sectors, the patterns are merged with the corresponding complete sector, the wildcard layer is dropped and the PCA parameters calculated using the combined pattern set. After this, only those patterns belonging to original wildcard sector are selected and stored, with the wildcard inserted back into the pattern. A similar approach is taken with the unlinked sectors without the added benefit of merging patterns to improve statistics.  
